...
%
%the parameters governing spiking of a population of neurons are $\bth^n=\{\bth^n_i\}_{i=1}^N$, where $\bth^n_i=\{\bw_i, b_i\}$,  $\bw_i=(w_{i1},\ldots, w_{iN})$, and $|\bth^n|=(1+N) N$. 
...
%Thus, the parameters governing the fluorescence observations are %$\bth^F=\{\bth^F_i\}_{i=1}^N$, where $\bth^F_i= $\{C^b_i, \tau^c_i, A_i, \sigma^c_i, \alpha_i, \beta_i, \gamma_i, \sigma^F_i\}$.  Let , and $|\bth^F|=8N$.  Therefore, the generative model is governed by $\bth=\{\bth^n, \bth^F\}$, where $|\bth|=(9+N)N$. 
