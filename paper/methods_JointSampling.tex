...
%As indicated by Eq. \eqref{eqn:bws}, to estimate the functional connectivity matrix, we require having posteriors such as $p[n_i(t), \bh(t) | \bF; \bth]$.  When using the above approximation, however, lack having joint samples of $n_i(t)$ and $h_j(t)$, for any $i\neq j$. But when the joint posterior approximately factorizes as above, we can sample $h_j(t)$ from the particle swarm for neuron $j$.  
...
% \begin{equation} \label{eqn:indep_approx2}
% 	P[n_i(t), \bh(t) |{\bf F};\theta]  \approx P[n_i(t)|F_i; \bth_i] \prod_{j=1}^N P[h_j(t)|F_j; \bth_j]. 
% \end{equation}
% 
% Given this approximate joint posterior, we use these approximately sufficient statistics to estimate the functional connection matrix, $\bw$.  
% 
