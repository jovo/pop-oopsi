function Z=spPY_GOOPSI(P,Y,X,t)
%HMM observation function P(Y|X,t) [for GOOPSI:Vogelstein-Paninski setup] 
...
if(isempty(Y))                    %DRAW FROM P(Y|X,t) 
  ...
  C=X(:,2);                       %[Ca]
  if(size(X,2)>=3) PG=X(:,3); else PG=ones(size(C)); end%rho probabilities
  
  if(isnan(Y)) Z=1./PG; return; end%missing observations
