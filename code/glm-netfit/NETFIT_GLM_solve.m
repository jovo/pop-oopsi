...
rate=par.rate;              % default spontaneous rate
...
par.scaleD=1/Sm;            % pass rescaling into GLM to adjust k-bounds
...
for k=range                           % FOR ALL NEURONS IN RANGE  
  tmp=n{k}';                          % compress to {patterns + frequencies}
  nf(1,:)=tmp(:);                     % see NETFIT_compress_cols for info
  ...
  lik_tot=lik_tot+lik;
  ivals(k,:)=bk/Sm;                   % write this as start point next iter
