...
for tr=1:1
    ...
    for i=1:Sim.Nc,                                 % infer spikes for each neuron
        ...
        fprintf('\nNeuron # = %g\n',i)
        [S M P] = GOOPSI_main_v3_0(S(i).F,I{i}.P,Tim);
        II{i}.S.n = S.n; II{i}.S.h = S.h; II{i}.M.nbar = M.nbar;
    end
    ...
    PlotMatrix(S,Sim,P,Phat)
    save(['SimConnector', num2str(Sim.T), '_', num2str(Sim.Nc), '_', num2str(tr)],'Phat','S','Sim','P')
...
function PlotMatrix(S,Sim,P,Phat)
...
for i=1:3
    plot(z1(S(i).F(tstart:tend))+1,'Color',col(i,:));
end
axis('tight'),
