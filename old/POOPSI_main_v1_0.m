function [S_best M_best E_best] = POOPSI_main_v1_0(F,P,Sim)
...
% S_best:   structure containing {H,w}_t^(i) for all t and i
...
%             subplot(414), plot(P.k)
    ...
    else                            % required for output of function
        S_best  = S;
        M_best  = M;                     
        E_best  = P;
        Sim.conv= true;
    ...
    E_best  = P;                    % update best parameters
    M_best  = M;                    % update best moments
    S_best  = S;                    % update best particles
end