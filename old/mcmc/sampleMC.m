...
for t=2:T
  ...
  if(sum(p(1:nh,t))==0)               %check for problems with forward Pr-field
    Xr=[]; xr=[];
    fprintf('Error: Degenerate forward probability field!\n'); 
    return;
  end  
...
for t=T-1:-1:1
  ...
  if(pf(i,t+1)==0)                    %check for problems with trans prob 
    Xr=[]; xr=[];
    fprintf('Error: P(x_{t+1}|Y_t)==0 in backward pass!\n'); 
    return;
  end
  ...
  if(sum(ph)==0)                      %check for problems with backward Pr-field
    Xr=[]; xr=[];
    fprintf('Error: Degenerate backward probability field!\n'); 
    return;
  end
  ph=cumsum(ph)/sum(ph); 
