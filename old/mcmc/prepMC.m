...
%Prepares sequence of states and observation from a HMM.
...
%PXX. Yuriy Mishchenko 2009 Columbia Un

T=length(X);
Yr=cell(1,T);
Xr=cell(1,T);

if(length(X)==1)            %if continuous HMM
  T=X;
...
else                        %if discrete HMM
  %INITIALIZE STATES P(x_1|Y_1)=P(Y_1|x_1)/Z
  p=funcPXX(P,X{1}); p=cumsum(p)/sum(p);%normalize
  r=rand; i=find(p>r,1);    %draw sample
  Xr{1}=X{1}(i,:); Yr{1}=funcPY(P,[],Xr{1},1);

  %FORWARD PASS
  for t=2:T
    p=funcPXX(P,X{t},Xr{t-1},t-1); p=cumsum(p)/sum(p);%normalize
    r=rand; i=find(p>r,1);  %draw sample
